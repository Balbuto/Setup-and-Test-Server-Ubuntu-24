...
run_verified_script() {
    ...
    if secure_download "$url" "$tmp" "$expected_sha"; then
        ...
        echo -e "${CROSS_MARK} Пропуск $name"
        return 1
    fi
}
...
ipv6_menu() {
    show_header
    echo -e "${WHITE}Управление IPv6${NC}\n"
    if sysctl -n net.ipv6.conf.all.disable_ipv6 2>/dev/null | grep -q 1; then
        echo -e "Статус: ${RED}Отключён${NC}"
    else
        echo -e "Статус: ${GREEN}Включён${NC}"
    fi
    echo ""
    echo "1) Отключить IPv6 (с бэкапом + restore-скрипт)"
...
base_setup() {
    show_header
    echo -e "${WHITE}БАЗОВАЯ НАСТРОЙКА - выберите шаги${NC}\n"
    ...
    if [[ $a_base =~ ^[Yy] ]]; then
        apt_install mc net-tools ncdu iftop curl wget git ca-certificates gnupg lsb-release
    fi
    if [[ $a_docker =~ ^[Yy] ]]; then
        if ! command -v docker &>/dev/null; then install_docker_apt; else echo "Docker уже установлен"; fi
    fi
...
manage_ufw() {
    show_header
    check_root || { pause_prompt; return; }
