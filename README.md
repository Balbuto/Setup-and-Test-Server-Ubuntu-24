...
SHA256: `9a4c5f8f7a3e8b6c47e1d2a9c0f5b8e3d6a7c9f1e2b5d8a0c3f6e9b2c5d8f0a1`

*хеш примерный — пересчитайте после релиза: `sha256sum server-setup.sh`*
...
### v3.2 — 2026-06-22 — bugfix release
- **APT полностью переписан**: `APT_OPTS` массив вместо строки, IFS-safe вывод пакетов
- **apt-lock wait** — ждёт освобождения `/var/lib/dpkg/lock`, не падает на свежей системе с unattended-upgrades
- **Стоп при ошибке apt** — базовая настройка прерывается, если `apt update/upgrade/install` упал, с показом хвоста лога в консоль
- **`docker --version` safe** — не валит лог если docker не поставился
- Все apt-вызовы через `"${APT_OPTS[@]}"`, вывод пакетов через `IFS=' '`
- `need_cmd`, тесты, `read` — везде `|| true`, меню не падает
...
При ошибке apt — настройка прерывается, в консоль выводится хвост лога.
...
Конфиг: `/etc/sysctl.d/99-server.conf`  
Limits: `/etc/security/limits.d/99-server.conf`
